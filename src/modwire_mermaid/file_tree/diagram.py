...
class ModwireFileTreeNode(ModwireDiagramContract):
    ...
    def validate_node(self):
        if not self.label.strip() or any(character in self.label for character in "\r\n"):
            raise ValueError("File-tree labels must be non-blank single lines")
        if self.description and any(character in self.description for character in "\r\n"):
            raise ValueError("File-tree descriptions must be single lines")
        if self.children and not self.is_directory:
            raise ValueError("File-tree files cannot contain children")
        if len(self.css_classes) != len(set(self.css_classes)):
            raise ValueError("File-tree CSS classes must be unique")
        return self
...
class ModwireFileTree(ModwireBaseDiagram):
    ...
    label_color: str = "currentColor"
    line_color: str = "currentColor"
    icon_color: str = "currentColor"
    description_color: str = "currentColor"
    highlight_background: str = "transparent"
    highlight_stroke: str = "currentColor"

    @model_validator(mode="after")
    def validate_tree(self):
        if not self.root.is_directory:
            raise ValueError("File-tree root must be a directory")
        self._validate_node(self.root)
        for comment in self.comments:
            if not comment.strip() or any(character in comment for character in "\r\n"):
                raise ValueError("File-tree comments must be non-blank single lines")
